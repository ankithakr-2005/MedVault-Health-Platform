...
A secure, full-stack digital health records web application built with **Python, Flask, MySQL, and Bootstrap**, featuring role-based access control (RBAC), secure file uploads, and encrypted authentication.
...
* **Relational Database Design:** Powered by **MySQL (InnoDB engine)** featuring a Many-to-Many junction table (`doctor_patients`) ensuring strict data isolation—doctors can only view medical records and manage prescriptions for patients explicitly assigned to their roster.
...
* **Database Engine:** MySQL / MariaDB (Managed via XAMPP)
